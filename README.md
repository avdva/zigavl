...
- `orderStorageByKey()` orders supported address-based cache storage by sorted key order. It is useful when a tree is built or bulk-mutated first and then queried many times by key or sorted position. After it succeeds, `get` uses binary search over dense storage in `O(logn)`, `at`, `iteratorAt`, and `deleteAt` resolve positions in `O(1)`, and iterator `next`/`prev` steps can advance through adjacent storage slots in `O(1)`. This layout is retained when a new maximum is appended or the current maximum is removed; other structural mutations invalidate it. The reordering itself is `O(n)`, and it may invalidate existing iterators, locations, entries, and value pointers. Currently this is supported by `.ArrayBased`, `.StableArrayBased`, and `.SplitArrayBased`.
- `buildFromSorted()` replaces the tree contents from strictly sorted unique `KV` items in `O(n)`. Invalid order or duplicate keys return `error.ItemsNotStrictlySorted` before the existing tree is cleared. If the temporary location list cannot be allocated, the existing tree is preserved; if allocating a replacement node fails, the tree is left empty. Supported address-based caches are stored in key order immediately after the build.
...
- `Entry.Value` points into the tree and can be used to update the stored value. `KV.Value` is an owned value.
